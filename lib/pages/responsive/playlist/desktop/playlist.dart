...
class DesktopPlaylistPage extends ConsumerStatefulWidget {
  @override
  DesktopPlaylistPageState createState() => DesktopPlaylistPageState();

  String id;
  DesktopPlaylistPage({required this.id, super.key});
}

class DesktopPlaylistPageState extends ConsumerState<DesktopPlaylistPage> {
  String id = "";
  bool loading = true;
  ...
  Widget build(BuildContext context) {
    ...
    //print("DesktopPlaylistPage: current width: ${ScreenBreakpoint.determine(MediaQuery.of(context).size.width.toInt())}");
    //print("DesktopPlaylistPage: width: ${MediaQuery.of(context).size.width.toInt()}");
                    ...
                    child: Column(
                      ...
                      children: [
                        if(!loading && init) ...[
                          ...
                          Text("${data.songs.length} song${songs.length == 1 ? "" : "s"}", style: Theme.of(context).textTheme.bodyLarge),
                          Text(data.description, style: Theme.of(context).textTheme.bodyMedium, overflow: TextOverflow.ellipsis),
                        ],
                        if(loading) ...[
                          Consumer(builder: (context, ref, child) { return const Skeletonizer(enabled: true, child: Text("Loading")); },),
              ...
              child: Column(
                ...
                children: [
                  ...
                  OutlinedButton(
                    ...
                    onPressed: () => playlistLogic(ref, context, id, "playlist"),
                  ),
                ],
              ),
            ),

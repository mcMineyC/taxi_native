...
class ArtistPage extends ConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    int count = ((MediaQuery.of(context).size.width / MediaCard.width).floor()) + (mobile ? 1 : 0);
          ...
          Container(
            ...
            child: albums.when(
              data: (data) => GridView.count(
                ...
                children: data.take(count*2).map((a) => MediaCard(text: a.displayName, image: a.imageUrl, thingId: a.id, thingType: "album", addedBy: a.addedBy)).toList(),
              ),
          ...
          Container(
            ...
            child: singles.when(
              data: (data) => GridView.count(
                ...
                // scrollDirection: Axis.vertical,
                childAspectRatio: MediaCard.width / (MediaCard.height+10),
                children: data.take(count*2).map((a) => MediaCard(text: a.displayName, image: a.imageUrl, thingId: a.id, thingType: "song", addedBy: a.addedBy)).toList(),
              ),
              loading: () => const Center(child: CircularProgressIndicator()),
              error: (_, __) => const Center(child: CircularProgressIndicator()),
            ),
